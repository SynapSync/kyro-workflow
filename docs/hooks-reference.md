...
Sprint Forge defines 12 lifecycle hooks that fire automatically during Claude Code sessions. Hooks are implemented as Node.js scripts that receive JSON via stdin and emit JSON via stdout, following the Claude Code hook protocol.
...
- Loads learned rules from `~/.sprint-forge/rules.md`
...
- Auto-captures identified learnings into `~/.sprint-forge/rules.md`
...
- Updates `~/.sprint-forge/rules.md` with confirmed rules
...
- References the specific rule from `~/.sprint-forge/rules.md`
...
```json
{
  ...
  "self_correction": {
    "enabled": true,
    "require_approval": true,
    "learned_file": "~/.sprint-forge/rules.md"
  }
}
```
...
process.stdin.on('end', () => {
  ...
  console.error('[SprintForge] Custom hook fired');
