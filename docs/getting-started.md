# Getting Started with Sprint Forge

Sprint Forge is a workflow system that orchestrates sprint-based project execution through specialized agents, lifecycle hooks, and persistent learning. This guide walks you through installation, first run, and core concepts.
...
- **Claude Code** -- Sprint Forge is built as a Claude Code plugin (also compatible with any agent via SkillKit)
...
This installs Sprint Forge as a Claude Code plugin with all commands, agents, hooks, and skills registered automatically.
...
3. Sprint Forge generates a sprint plan with phases and tasks
...
After running `/forge` (INIT mode), Sprint Forge creates a project workspace:
...
Global data is stored in `~/.sprint-forge/`:
...
| `~/.sprint-forge/rules.md` | Learned rules that apply across all projects. Loaded automatically at session start. |
...
Sprint Forge operates in three modes, determined by your intent:
...
Gates are mandatory approval checkpoints between phases. Sprint Forge never proceeds past a gate without your explicit approval. There are three gates in the `/forge` cycle:
